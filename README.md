...
│
├── promo-config.js         프로모션 팝업 문구·기간 설정 (메인 페이지 전용)
├── promo.js                프로모션 팝업 동작 로직
│
├── assets/                 로고, OG 공유 이미지
...
메인 페이지(`/`)에만 뜨는 중앙 팝업입니다. 다른 페이지에는 적용되어 있지 않습니다.

| 파일 | 역할 |
|---|---|
| `promo-config.js` | **문구·기간·켜고 끄기.** 내용을 바꿀 때는 이 파일만 고치면 됩니다 |
| `promo.js` | 팝업 동작 로직. 평소에는 건드릴 일이 없습니다 |

**뜨는 조건** (셋 다 맞아야 뜹니다)

1. `enabled` 가 `true`
2. 오늘 날짜가 `startDate` ~ `endDate` 사이 (종료일 당일까지 노출)
3. 방문자가 '오늘 하루 안 보기' 로 숨기지 않았을 것

**급하게 내려야 할 때**는 `promo-config.js` 의 `enabled` 를 `false` 로 바꿔 `main` 에 푸시하면 즉시 사라집니다.
기간이 끝나면 아무것도 하지 않아도 자동으로 사라지므로, 프로모션이 끝났는데 계속 떠 있는 사고는 나지 않습니다.

**로컬에서 미리 보기** — 위 조건을 전부 무시하고 항상 띄웁니다.

```
http://localhost:8000/?promo=preview
```

**방문자가 닫는 방법** — X 버튼 / 하단 '닫기' / 배경 클릭 / ESC 키.
'오늘 하루 안 보기' 를 누르면 그 브라우저에서 자정까지 다시 뜨지 않습니다. (기간은 `hideDays` 로 조정)

**성과 확인** — Google Analytics 4 에 다음 이벤트가 기록됩니다.

- `promo_view` : 팝업이 노출됨
- `promo_click` : 신청 버튼을 누름
- `promo_close` : 팝업을 닫음 (`close_reason` 으로 어떻게 닫았는지 구분)
...
이 레포(`aqbar-website-v3`)가 **현재 운영 중인 유일한 소스**입니다.

- `aqbar-website-v1` — 이전 버전. private 으로 전환해 보관만 하고 있습니다
- `aqbar-website-v2` — 이전 버전. private 으로 전환해 보관만 하고 있습니다

두 레포는 **더 이상 사용하지 않습니다.** 수정은 항상 이 레포에서만 하세요.
